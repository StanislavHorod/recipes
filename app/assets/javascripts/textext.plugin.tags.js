...
(function($)
{
	...
	p.init = function(core)
	{
		...
		if(self.opts(OPT_ENABLED))
		{
			...
			self.on({
			  ...
			  'keypress'			 :function(e){
					console.log(e);
					if(e.charCode == 44 || e.charCode == 59 || e.charCode == 46){
						var self = this,
							val  = self.val(),
	...
	p.onGetFormData = function(e, data, keyCode)
	{
		var self       = this,
			...
			charCode = self.val().slice(-1).charCodeAt(0);
			//console.log();
			if(keyCode === 13 || charCode === 44 || charCode === 46 || charCode === 59) inputValue = ''
			else inputValue = self.val();
			//console.log(keyCode);
	...
	p.onEnterKeyPress = function(e)
	{
		...
		if(self.isTagAllowed(tag))
		{
			...
			$('.text-wrap textarea').attr('placeholder','');
			//console.log($('.text-wrap input').attr('placeholder'));
		}
	};
	...
	p.updateFormCache = function()
	{
		...
		self.tagElements().each(function()
		{
			...
			str_result += $(this).data(CSS_TAG)+', ';

		});

		// cache the results to be used in the onGetFormData
		self._formData = str_result.slice(0, -2);//result;
